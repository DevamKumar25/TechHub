    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-xs md:flex-row md:items-center md:justify-between">
        ...
        <div className="relative flex-1 max-w-lg">
          ...
          <input
            type="text"
            placeholder="Search by client ID, name, email, company..."
            value={search}
            onChange={(e) => {
          ...
          <table className="w-full min-w-[1000px] border-collapse text-left text-sm text-slate-700">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                clients.map((clt) => (
                  <tr key={clt._id} className="hover:bg-slate-50/50 transition-colors">
                    {/* Client Name */}
                    ...
                    {/* Status */}
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex rounded-full border px-2.5 py-0.5 text-xs font-semibold leading-5 ${getStatusClass(clt.status)}`}>
                        {clt.status}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4 text-right relative whitespace-nowrap">
                      <button
                        onClick={() =>
                          setActiveDropdown(activeDropdown === clt._id ? null : clt._id)
                        }
                      ...
                      {activeDropdown === clt._id && (
                        <>
                          ...
                          <div className="absolute right-6 top-12 z-20 w-32 rounded-xl border border-slate-100 bg-white py-1 shadow-lg ring-1 ring-black/5">
                            <button
                              onClick={() => navigate(`/clients/edit/${clt._id}`)}
